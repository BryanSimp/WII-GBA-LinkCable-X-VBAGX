...
void printmain()
{
	...
	printf("Visual Boy Advance GX support added by BryanSimp\n \n");
}
...
int main(int argc, char *argv[]) 
{
	...
	mkdir("/vbagx", S_IREAD | S_IWRITE);
	mkdir("/vbagx/saves", S_IREAD | S_IWRITE);
	mkdir("/vbagx/roms", S_IREAD | S_IWRITE);
	if(!dirExists("/dumps"))
	{
		printmain();
	...
	{
		printmain();
		printf("Connect GBA with no game in port 2\n");
			...
			while(1)
			{
				...
				if((btns&PAD_BUTTON_A)||(wbtns&WPAD_BUTTON_A)) 
				{
					if(recv() == 0) //ready
					{
						...
						//generate file paths
						char gamename[64];
						sprintf(gamename,"/vbagx/roms/%.12s [%.4s%.2s].gba",
							(char*)(testdump+0xA0),(char*)(testdump+0xAC),(char*)(testdump+0xB0));
						fixFName(gamename + strlen("/vbagx/roms/")); // Only fix the filename part
						char savename[64];
						sprintf(savename,"/vbagx/saves/%.12s [%.4s%.2s] Auto.sav",
							(char*)(testdump+0xA0),(char*)(testdump+0xAC),(char*)(testdump+0xB0));
						fixFName(savename + strlen("/vbagx/saves/")); // Only fix the filename part
	...
	//Check if Gamecube valid input given before checking Wiimote
						...
						if (command == 2)
						{
							...
							{
								fclose(f);
								printf("Save already exists. Press A to overwrite.\n");
								printf("Press B to cancel.\n");

								int overwriteCommand = 0; // New variable to track user decision
								bool overwriteUndecided = true;

								while (overwriteUndecided)
								{
									PAD_ScanPads();
									WPAD_ScanPads();
									VIDEO_WaitVSync();
									...
									switch (btns)
									{
										...
										case PAD_BUTTON_A:
											overwriteCommand = 1;  // Confirm overwrite
											overwriteUndecided = false;
											break;

										case PAD_BUTTON_B:
											overwriteCommand = 0;  // Cancel
											overwriteUndecided = false;
											break;
									}

									if (!overwriteUndecided)
										break;
								...
								// Prevent execution if user pressed B
								if (overwriteCommand == 0)
								{
									printf("Save file was not replaced.\n");
									command = 0;  // Reset command to prevent unintended execution
								}
							}
						}
						if(command == 3)
